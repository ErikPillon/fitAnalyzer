...
with col2:
    act2_name = st.selectbox("Select Activity 2", activity_names, index=min(1, len(activity_names)-1))
...
if act1_name and act2_name:
    if act1_name == act2_name:
        ...
        if act1 and act2:
            type1 = act1.get("activity_type")
            type2 = act2.get("activity_type")
            
            if type1 != type2:
                st.error(f"Cannot compare activities of different types: {type1} vs {type2}")
            else:
                st.subheader(f"Comparison: {str(type1).capitalize()}")
                
                # Basic metrics
                col_m1, col_m2, col_m3, col_m4 = st.columns(4)
                
                with col_m1:
                    st.metric("Duration (min)", f"{act1.get('duration_min', 0):.1f}", 
                              f"{act1.get('duration_min', 0) - act2.get('duration_min', 0):.1f}")
                with col_m2:
                    st.metric("Distance (m)", f"{act1.get('distance', 0):.1f}", 
                              f"{act1.get('distance', 0) - act2.get('distance', 0):.1f}")
                with col_m3:
                    st.metric("Avg Heart Rate", f"{act1.get('avg_heart_rate', 0):.0f}", 
                ...
                def create_comparison_df(field, act1, act2):
                    ...
                    df1 = pd.DataFrame({field: data1, 'Activity': act1_name, 'Index': range(len(data1))})
                    df2 = pd.DataFrame({field: data2, 'Activity': act2_name, 'Index': range(len(data2))})
                ...
                if lat1 and lon1 and lat2 and lon2:
                    ...
                    if not df_coords.empty:
                        fig = px.scatter_mapbox(df_coords, lat="lat", lon="lon", color="Activity", zoom=12, height=500)
                ...
                with col_chart1:
                    ...
                    if act1.get("heart_rate") and act2.get("heart_rate"):
                        ...
                        fig = px.line(df_hr, x="Index", y="heart_rate", color="Activity", title="Heart Rate over Time")
                        st.plotly_chart(fig, use_container_width=True)
                ...
                with col_chart2:
                    ...
                    if act1.get(speed_col) and act2.get(speed_col):
                        ...
                        fig = px.line(df_speed, x="Index", y=speed_col, color="Activity", title="Speed over Time (m/s)")
                        st.plotly_chart(fig, use_container_width=True)
                ...
                with col_chart3:
                    # Altitude
                    alt_col = "enhanced_altitude" if act1.get("enhanced_altitude") else "altitude"
                    if act1.get(alt_col) and act2.get(alt_col):
                        st.subheader("Altitude Comparison")
                        df_alt = create_comparison_df(alt_col, act1, act2)
                        fig = px.line(df_alt, x="Index", y=alt_col, color="Activity", title="Altitude over Time")
                        st.plotly_chart(fig, use_container_width=True)
                ...
                with col_chart4:
                    ...
                    if act1.get("power") and act2.get("power"):
                        ...
                        fig = px.line(df_pwr, x="Index", y="power", color="Activity", title="Power over Time")
                        st.plotly_chart(fig, use_container_width=True)
                    elif act1.get("cadence") and act2.get("cadence"):
                        st.subheader("Cadence Comparison")
                        df_cad = create_comparison_df("cadence", act1, act2)
                        fig = px.line(df_cad, x="Index", y="cadence", color="Activity", title="Cadence over Time")
                        st.plotly_chart(fig, use_container_width=True)
